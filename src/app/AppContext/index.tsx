import React, {
  ReactNode,
  useState,
  createContext,
  Dispatch,
  SetStateAction,
  useEffect,
  useCallback,
} from 'react';
import { prepareAudioElement } from '../services/prepareAudioElement';
import { CAROUSEL_SINGLE_ROTATION_ANGLE } from '../services/const';
import Vibrant from 'node-vibrant';
import { ParsedVibrantColors, parseVibrantColors } from '../services/parseVibrantColors';
import { isNode } from '../services/isNode';

export interface AppContextState {
  actions: {
    setAudio: Dispatch<SetStateAction<HTMLAudioElement>>;
    setCarouselRotation: (value: number) => void;
    toggle: () => void;
    next: () => void;
    previous: () => void;
  };
  state: {
    audio?: HTMLAudioElement;
    currentTime: number;
    carouselRotation: number;
    vibrantColors?: ParsedVibrantColors;
  };
}

const defaultValues: AppContextState = {
  actions: {
    setAudio: () => {},
    setCarouselRotation: () => null,
    toggle: () => null,
    next: () => null,
    previous: () => null,
  },
  state: {
    audio: prepareAudioElement(),
    currentTime: 0,
    carouselRotation: 0,
  },
};

export const AppContext = createContext<AppContextState>(defaultValues);

export interface Props {
  children: ReactNode;
  initialState: {
    audio?: HTMLAudioElement;
    currentTime: number;
    carouselRotation: number;
    vibrantColors?: ParsedVibrantColors;
  };
}
export const ContextProvider = ({ initialState, children }: Props) => {
  const [audio, setAudio] = useState<HTMLAudioElement>();
  const [currentTime, setCurrentTime] = useState(0);
  const [carouselRotation, setCarouselRotation] = useState(0);
  const [vibrantColors, setVibrantColors] = useState<ParsedVibrantColors>();

  const getColors = useCallback(async () => {
    if (!isNode) {
      const palette = await Vibrant.from(
        'data:image/jpeg;base64,',
      ).getPalette();
      setVibrantColors(parseVibrantColors(palette));
    }
  }, []);

  useEffect(() => {
    getColors();
    setAudio(prepareAudioElement(setCurrentTime));
  }, []);

  const toggle = () => {
    if (audio && !audio.paused) {
      audio.pause();
      return;
    }
    if (audio) {
      audio.play();
    }
  };

  const next = () => {
    setCarouselRotation(carouselRotation + CAROUSEL_SINGLE_ROTATION_ANGLE);
  };
  const previous = () => {
    setCarouselRotation(carouselRotation - CAROUSEL_SINGLE_ROTATION_ANGLE);
  };
  
  return (
    <AppContext.Provider
      value={{
        actions: {
          setAudio: setAudio as any,
          setCarouselRotation,
          toggle,
          next,
          previous,
        },
        state: {
          carouselRotation,
          audio,
          currentTime,
          vibrantColors,
        },
      }}
    >
      {children}
    </AppContext.Provider>
  );
};
